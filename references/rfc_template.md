...
|---|-------|-------|-----------------|
| 1 | 背景 | Review | Context + motivation |
| 2 | 用户痛点 | Review | User-perceivable problems (no internal terms) |
| 3 | 目标与非目标 | Review | Verifiable goals + success criteria table + glossary |
| 4 | 一页结论 | Review | TL;DR + impact table + must-pass + quality summary + reading guide |
| 5 | 关键决策与取舍 | Normative | DEC-### + Unresolved (Hard/Soft) |
| 6 | 方案概览 | Review | End-to-end flow + contracts + API design (if applicable) + diagrams A/B/C with Notes |
| 7 | 影响分析与兼容性 | Review | API behavior + API compatibility + breaking changes + rollback + evidence |
...
| 13 | 门禁声明 | Appendix | Trigger YES/NO + Links (Gate-A checks this) |
| 14 | 发布与影响元信息 | Appendix | Release & impact metadata |
| 15 | 附录 | Appendix | Roles + 11-item self-check + document meta |
...
### 6.1 端到端主链路
> 描述数据流向与决策点，使用功能角色名而非实现类名。
> ❌ "调用 ModuleA.checkStatus()"
> ✅ "在数据分发前执行状态判定（O(1) 查询）"
...
> 线程安全约束应描述并发安全要求（如"无数据竞争"、"TOCTOU 安全"），
> 不应指定同步原语（mutex、RWLock、atomic 等）。
> 同步策略属于实现决策，不属于设计契约。

### 6.3 设计图
> 图中组件框应标注功能角色（如"状态管理模块"），不使用源代码类名。
> 性能/容量约束可标注在组件上（如"O(1) 查询"）。
> 图必须反映端到端方案。每张图后跟随 1–8 条文本要点。要点应补充图中无法表达的信息（性能约束、安全含义、时序不变量），而非重述图中已有的拓扑或调用关系。
> ❌ 重述拓扑："组件 A 作为调用方位于系统服务信任域内"（图上已有标注）
> ✅ 补充含义："组件 A 位于系统信任域，因此无需额外鉴权（符合 SEC-HR-###）"
...
### 7.1 对外 API 行为
> 使用统一矩阵替代分离的"不变/变化"列表，便于直接对照。

| API / 组件 | 当前行为 | 变更后行为 | 是否变化 | 不变项说明 | 是否 Breaking | 默认值策略 |
|---|---|---|---|---|---|---|
| <...> | <...> | <...> | 不变/变化 | <不变则注明"保持不变(unchanged)"；变化则留空> | <...> | <...> |
...
### 7.1.3 API 契约兼容性（适用于对外 API 变更）
> 若 §6.4 标注 N/A，本节同样标注 N/A。
...
> ℹ️ 不变量/禁止状态已合并到 §9.1 规则的 Postcondition 子弹点中，不再独立列出。
...
- [ ] 若涉及对外 API：接口规格 + 开发者模型 + 错误码 + 版本策略 + 兼容性（§6.4 + §7.1.3）
- [ ] 去重：每项行为事实有且仅有一个权威位置。如同一约束在 §6.2 和 §8/§9 中重复出现，应合并或转为反向引用
- [ ] 实现隔离：§6 中不包含实现类名、方法签名、同步原语名称或特定编程语言容器类型。使用功能角色名和行为契约
