...
| DEC-### | 决策与 trade-off | DEC-003: 选 B，延迟更低（牺牲内存） |
...
| Tier 2 | 方向分叉——≥2 条路径各有 trade-off | **必须问**，答案决定设计方向 |
...
- **Gate-B（语义）**：多角色并行评审，检查约束充分性、trade-off 合理性、场景完整性。6 条 PASS 谓词全部满足才通过
